...
//----------------------------------------------------------------------------------------------------
        ...
        for (i = 0; i < barColors.length; i++) {
            ...
            if (this.legendAbsValues) {
                ...
                for (chart in barValues) {
                    for (label in barValues[chart])
                        sum += barValues[chart][label][i];
                }
    ...
    this.create_hBar = function (value, percent, mPerc, mPerc_neg, max_neg, mul, valSpace, bColor, border, spacer, spacer_neg) {
        ...
        if (percent < 0) {
            ...
            if (this.showValues < 2)
                bar += '<span style="' + this.cssPERCVALUES + '">' + _number_format(percent, this.percValuesDecimals) + '%<\/span>';
            ...
            if (percent) {
                ...
                bar += '<td width=1 height=' + (this.barWidth + (border * 2)) + '><\/td>';

            bar += '<td style="' + this.cssPERCVALUES + '" width=' + Math.round((mPerc - percent) * mul + valSpace) + ' align=left nowrap>';
            if (this.showValues < 2)
                bar += '&nbsp;' + _number_format(percent, this.percValuesDecimals) + '%';
            bar += '&nbsp;<\/td>';
        }
        bar += '<\/tr><\/table>';
    ...
    this.create_vBar = function (value, percent, mPerc, mPerc_neg, max_neg, mul, valSpace, bColor, border, spacer, spacer_neg) {
        ...
        if (percent < 0) {
            ...
            if (percent) {
                ...
                bar += '<\/tr><tr><td width=' + (this.barWidth + (border * 2)) + ' height=1><\/td>';
            if (max_neg) {
    ...
    this.create = function () {
        ...
        if (this.titles)
            var t = (typeof (this.titles) == 'string') ? this.titles.split(',') : this.titles;
        else
            var t = [];
        if (this.labels)
            var r = (typeof (this.labels) == 'string') ? this.labels.split(',') : this.labels;
        else
            var r = [];
        var label = graph = bColor = '';
        var percent = value = rowspan = colspan = 0;
        if (this.barColors)
            var drc = (typeof (this.barColors) == 'string') ? this.barColors.split(',') : this.barColors;
        else
            var drc = [];
        var drv, val = [];
        var bc = [];
        ...
        if (this.barLevelColors) {
            for (i = 0; i < this.barLevelColors.length; i += 2) {
                if (this.barLevelColors[i] == 'MAX')
                    this.barLevelColors[i] = max;
            }
        }
            ...
            if (this.type == 'hbar') {
                ...
                for (i = 0; i < val[chart].length; i++, lcnt++) {
                    ...
                    for (j = 0; j < val[chart][i].length; j++) {
                        ...
                        graph += '<\/td><\/tr>';
                        if (j < val[chart][i].length - 1)
                            graph += '<tr>';
                    }
                    if (this.labelSpace && i < val[chart].length - 1)
                ...
                for (i = 0; i < val[chart].length; i++, lcnt++) {
                    ...
                    if (this.type == 'fader')
                        graph += this.build_fader(value, Math.round(this.barWidth / 2), this.barWidth, Math.round(percent * mul), bColor);
                    else
                        graph += this.build_bar(value, Math.round(percent * mul), this.barWidth, bColor);
                    graph += '<\/td><td width=' + Math.round((100 - percent) * mul) + '><\/td>';
                    graph += '<\/tr><\/table><\/td>';
                    if (this.showValues < 2)
                        graph += '<td style="' + this.cssPERCVALUES + '" nowrap>&nbsp;' + _number_format(percent, this.percValuesDecimals) + '%<\/td>';
                    graph += '<\/tr>';
                    if (this.labelSpace && i < val[chart].length - 1)
...
function _number_format(val, dec) {
    if (dec) {
        if (val == 0) {
            ...
            if (val.length <= dec)
                for (var i = 0; i < dec - val.length + 1; i++)
                    val = '0' + val;
